...
async function getSessions() {
  ...
  const res = await fetch(`${baseUrl}/api/sessions?limit=50&sort=-createdAt`, {
    ...
    <div>
      <div className="flex items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-600">Healing session offerings</p>
        </div>
        <Link
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="grid grid-cols-12 gap-4 px-6 py-3 text-sm font-semibold text-gray-600 bg-gray-50 border-b border-gray-200">
          ...
          <div className="col-span-4">Active</div>
        </div>
        <div className="divide-y divide-gray-100">
          {docs.length === 0 ? (
            <div className="px-6 py-10 text-gray-600">No sessions found.</div>
          ) : (
            docs.map((doc: any) => (
              <div key={doc.id} className="grid grid-cols-12 gap-4 px-6 py-4">
                ...
                <div className="col-span-3 text-gray-700">{doc.duration || ''}</div>
                <div className="col-span-4 text-gray-700">{doc.isActive ? 'Yes' : 'No'}</div>
              </div>
            ))
          )}
        </div>
